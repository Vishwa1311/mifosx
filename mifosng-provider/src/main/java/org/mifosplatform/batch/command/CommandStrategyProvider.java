...
public class CommandStrategyProvider {
    ...
    private final ConcurrentHashMap<CommandContext, String> commandStrategies = new ConcurrentHashMap<CommandContext, String>();
    ...
    private void init() {
        ...
        this.commandStrategies.put(CommandContext.resource("savingsaccounts\\/\\d+\\/\\D+").method("POST").build(),
                "SavingsAccountTransactionCommandStrategy");
        this.commandStrategies.put(CommandContext.resource("journalentries").method("POST").build(), "CreateJournalEntryCommandStrategy");
        this.commandStrategies.put(CommandContext.resource("accounttransfers").method("POST").build(), "AccountTransafersCommandStrategy");
    }
